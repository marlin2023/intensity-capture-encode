...
AVStream * add_video_stream(AVFormatContext *fmt_ctx, enum CodecID codec_id,
		Output_Context *ptr_output_ctx, int prog_no) {
...
AVStream * add_audio_stream(AVFormatContext *fmt_ctx, enum CodecID codec_id,
...
int init_output(Output_Context *ptr_output_ctx, char* output_file, int prog_no) {
	...
	//bind
	avpicture_fill((AVPicture *) ptr_output_ctx->encoded_yuv_pict,
			ptr_output_ctx->pict_buf,
...
static void open_video(Output_Context *ptr_output_ctx, AVStream * st,
	...
	if (prog_no == 0) { /*low */
		...
		av_dict_set(&opts, "level", "21", 0); //set level
		...
		av_dict_set(&opts, "psy_rd", "1.00:0.2", 0);
...
//		av_dict_set(&opts, "partitions", "all", 0);
		...
		av_dict_set(&opts, "mbtree", "1", 0);
//		av_dict_set(&opts, "aq_mode", "2", 0);
//		av_dict_set(&opts, "aq_strength", "0.89", 0);

		//connect the string content x264opts
//		:vbv-bufsize=600:vbv-maxrate=600
		av_dict_set(&opts, "x264opts",
				"ref=3:me=umh:bframes=3:b-adapt=1:force-cfr=1", 0);
...
//		av_dict_set(&opts, "aq_strength", "0.89", 0);
		//connect the string content x264opts
		av_dict_set(&opts, "x264opts",
				"ref=6:me=umh:bframes=4:b-adapt=1:force-cfr=1", 0);
		...
		av_dict_set(&opts, "psy", "1", 0);
		av_dict_set(&opts, "psy_rd", "1.00:0.2", 0);
		av_dict_set(&opts, "mixed_refs", "1", 0); //mixed references
		av_dict_set(&opts, "fast_pskip", "0", 0);
		av_dict_set(&opts, "b_pyramid", "0", 0);

		av_dict_set(&opts, "rc_lookahead", "80", 0);
//		av_dict_set(&opts, "partitions", "all", 0);
		av_dict_set(&opts, "dct8x8", "1", 0);
		av_dict_set(&opts, "mbtree", "1", 0);
//		av_dict_set(&opts, "aq_mode", "2", 0);
//		av_dict_set(&opts, "aq_strength", "0.89", 0);
		//connect the string content x264opts
		av_dict_set(&opts, "x264opts",
				"ref=6:me=umh:bframes=6:b-adapt=1:force-cfr=1", 0);
	}
...
//	duration = FFMAX(duration, 1/av_q2d(ptr_input_ctx->ptr_format_ctx->streams[ptr_input_ctx->video_index]->avg_frame_rate));
	...
	sync_ipts = ptr_output_ctx->sync_ipts
			/ av_q2d(ptr_output_ctx->video_stream->codec->time_base);
...
//			return;
		...
		if (video_encoded_out_size > 0) {
			...
			ptr_output_ctx->pkt.stream_index =
					ptr_output_ctx->video_stream->index;
...
//			av_write_frame(ptr_output_ctx->ptr_format_ctx, &ptr_output_ctx->pkt);
			av_interleaved_write_frame(ptr_output_ctx->ptr_format_ctx,
					&ptr_output_ctx->pkt);
...
void encode_audio_frame(Output_Context *ptr_output_ctx1[], uint8_t *buf,
		int buf_size, int prog_num) {
	...
	if (pkt.size) {
		...
		for (i = 0; i < prog_num; i++) {
			...
			av_interleaved_write_frame(ptr_output_ctx1[i]->ptr_format_ctx,
					&pkt);
		...
		for (;;) {
			...
			case AVMEDIA_TYPE_VIDEO: {
				int nEncodedBytes = avcodec_encode_video(
						ptr_output_ctx->video_stream->codec,
						ptr_output_ctx->video_outbuf,
						ptr_output_ctx->video_outbuf_size, NULL);
...
void do_audio_out(Output_Context *ptr_output_ctx1[], void * src_audio_buf,
		int src_audio_buf_size, int nb_sample, int prog_num) {
...
//	AVCodecContext *dec = ptr_input_ctx->audio_codec_ctx;
	...
	/*need_realloc:*/
	audio_buf_size = (allocated_for_size
			+ isize * /*dec->channels*/CAPTURE_AUDIO_CHANNEL_NUM - 1)
			/ (isize * CAPTURE_AUDIO_CHANNEL_NUM);
	audio_buf_size = (audio_buf_size * enc->sample_rate
			+ /*dec->sample_rate*/CAPTURE_AUDIO_SAMPLE_RATE)
			/ CAPTURE_AUDIO_SAMPLE_RATE;
	...
	av_fast_malloc(&(ptr_output_ctx->audio_buf),
			&(ptr_output_ctx->allocated_audio_buf_size), audio_buf_size);
	...
	if (enc->channels != CAPTURE_AUDIO_CHANNEL_NUM
			|| enc->sample_fmt
					!= dec_sample_fmt || enc->sample_rate!= CAPTURE_AUDIO_SAMPLE_RATE
	...
	if (ptr_output_ctx->audio_resample && !ptr_output_ctx->swr) {
		...
		ptr_output_ctx->swr = swr_alloc_set_opts(NULL, enc->channel_layout,
				...
				dec_layout_channel, dec_sample_fmt, CAPTURE_AUDIO_SAMPLE_RATE,
				0, NULL);

		if (av_opt_set_int(ptr_output_ctx->swr, "ich",
				CAPTURE_AUDIO_CHANNEL_NUM, 0) < 0) {
		...
		if (!ptr_output_ctx->swr) {
			av_log(NULL, AV_LOG_FATAL,
					...
					CAPTURE_AUDIO_CHANNEL_NUM, CAPTURE_AUDIO_SAMPLE_RATE,
